...
function chunksInRange(pos, rad) {
	...
	for (let r = 0; r*chunkSize <= height; r++) {
		for (let c = 0; c*chunkSize <= width; c++) {
			let [y, x] = [r * chunkSize, c * chunkSize]
			// x, y is top left
...
class Organism {
	...
	tick() {
		if (this.vel.magSq() > 0) {
			...
			this.pos.add(p5.Vector.mult(this.vel, elapsedTicks()).limit(this.topspeed))
...
class WhiteBlood extends Organism {
	...
	tick() {
		if (!this.target || !entities[this.target.r][this.target.c].has(this.target)) {
			let toCheck = []
			for (let [r, c] of chunksInRange(this.pos, this.sight))
				if (entities[r] && entities[r][c]) toCheck = toCheck.concat([...entities[r][c]])
			let tar = toCheck
	...
	updatePos() {
		super.updatePos()
		let toCheck = []
		for (let [r, c] of chunksInRange(this.pos, 50))
			if (entities[r] && entities[r][c]) toCheck = toCheck.concat([...entities[r][c]])
		toCheck = toCheck.filter(e => (e instanceof Bacteria) && (p5.Vector.dist(e.pos, this.pos) <= 7.5 + e.size/2))
